    ...
    return {
      ...
      handleSwipe() {
        ...
        if (Math.abs(deltaX) > 50) {
          if (deltaX < 0) {
            window.location.href = "{{ url('/certifications') }}";
          } else {
...
<div id="vanta-bg" class="min-h-screen relative" x-data="swipeComponent()" x-init="initSwipe()">
...
<div class="absolute top-[90px] left-6 z-50">
  ...
  <ul x-show="navOpen" x-cloak x-transition
    ...
    <li><a @click="navOpen = false" href="{{ url('/projects') }}" class="block text-purple-400">Projects</a></li>
